...
TProduceFunctionMajatnik=class(TProduceFunction)
private
      FAlfa: real;
      procedure OnCalcValues; override;
      function GetAlfa: real;
      procedure SetAlfa(AValue: real);
public
      constructor Create(AAlfa,AMax,AMin,ADelta: real); overload;
      constructor Create; overload;
      destructor Destroy; override;
published
     property AlfaFunction: real read GetAlfa write SetAlfa;
end;
...
procedure TProduceFunctionMajatnik.SetAlfa(AValue: real);
begin
   try
      if (AValue<(-1)*VPI) or (AValue > VPI) then
      begin
          FAlfa:=DEFAULT_BETA;
          Raise Exception.Create('Неправильные значение растояния начального угла alfa (между -180 и 180 град)!');
      end
      else FAlfa:=AValue;
      OnCalcValues;
   except
...
//---------------------------------------------------------------------------
function TProduceFunctionMajatnik.GetAlfa: real;
begin
    result:=FAlfa;
end;
...
procedure TProduceFunctionMajatnik.OnCalcValues;
var
  i: integer;
  PValue: ^real;
...
begin
  ...
  begin
     ...
     begin
       ...
       New(temp);
       temp.x:=PValue^;
       v1:=v0-Delta*sin(alfan);
       alfan:=alfan+Delta*v1;
       v0:=v1;
       temp.y:=alfan*360/(2*VPI);
...
constructor TProduceFunctionMajatnik.Create(AAlfa,AMax,AMin,ADelta: real);
begin
  try
     FFunctionValues:=TList.Create;
     FAlfa:=AAlfa;
...
//---------------------------------------------------------------------------
constructor TProduceFunctionMajatnik.Create;
begin
  try
     FFunctionValues:=TList.Create;
     FAlfa:=DEFAULT_BETA;
...
//---------------------------------------------------------------------------
destructor TProduceFunctionMajatnik.Destroy;
var
   i: Integer;
begin
   if Count<>0 then
...
begin
   ...
   temp:=Height-temp;
   FCanvas.TextOut(FOffset.x+x-10,FOffset.y+trunc(temp)-10,'A');
...
begin
  ...
  if not FPrecisionDiv then
   ...
   FCanvas.TextOut(FOffset.x+trunc(temp)-10,FOffset.y+y-20,'Tau');
 ...
 if Edit1.Modified then
    begin
       try
          ...
          pfr.AlfaFunction:=t*2*VPI/360;
          sc.UpdateGrafics;
       except
         ShowMessage('Неправильное значение начального угла альфа !');
         Edit1.SetFocus;
       end;
    end;
end;
...
begin
   pnt.X:=40;
   pnt.Y:=40;
   pfr:=TProduceFunctionMajatnik.Create(1,10,0,0.001);
   Edit1.Text:=FloatToStr(pfr.AlfaFunction*360/(2*VPI));
  ...
  begin
         ...
         begin
            ...
            begin
                      ...
                      begin
                         Image1.Hint := 'Tau='+FloatToStr(SimpleRoundTo(p1.x, -1*l))+';Alfa='+FloatToStr(p1.y);
